...
def setpassword():
    ...
    def complete():
        ...
        setapass.destroy()
    setapass.geometry("400x100")
    if curlang.get() == 0:
        setpasslabel.set("Set a password:")
        setpassbuttonlabel.set("Set password")
        setapass.title("Set A Password!")
    elif curlang.get() == 1:
        setpasslabel.set("Назначьте пароль:")
        setpassbuttonlabel.set("Назначить пароль")
        setapass.title("Назначьте Пароль!")
    elif curlang.get() == 2:
        setpasslabel.set("Βάλτε κωδικό:")
        setpassbuttonlabel.set("ΟΚ")
        setapass.title("Βάλτε Κωδικό!")
    blanklabel = Label(setapass, text = " ", bg='#bdbdbd').grid(row = 0)
    label = Label(setapass, text = setpasslabel.get(), bg='#bdbdbd').grid(row = 1)
    e.grid(row = 1, column = 1)
    setpass = Button(setapass, text = setpassbuttonlabel.get(), command = complete, width = 15, bg='lightgrey', borderwidth = 5).grid(row = 1, column = 2)
setpassword()
...
def openpasses():
    ...
    refreshbutton = Button(savedpassespage, textvariable = refreshbuttonlabel, bg='lightgrey', command = refreshpasses, borderwidth = 5).grid(row = 0, column = 1)
    savepassesbutton = Button(savedpassespage, textvariable = savepassesbuttonlabel, bg='lightgrey', command = areyousure, borderwidth = 5).grid(row = 2, column = 0)
    refreshpasses()
...
b1 = Button(master, textvariable = generatebutton, command = dothis, bg='lightgrey', borderwidth = 5).grid(row=1, column=3, ipadx = 1, stick = W)
...
Last edit made at 5/4/2022 03:45
"""
