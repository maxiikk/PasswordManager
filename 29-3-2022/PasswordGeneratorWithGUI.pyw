...
def openpasses():
    ...
    def savepassesfile(areyousure):
        if os.path.isfile("Encrypted Password Storage.txt"):
            ...
            sfile.truncate(0)
            sfile.write(passesl2.get('1.0', "end"))
            sfile.close()
    def refreshpasses():
    ...
    refreshbutton = Button(savedpassespage, textvariable = refreshbuttonlabel, bg='#bdbdbd', command = refreshpasses).grid(row = 2, column = 0)
    if curlang.get() == 0:
...
def setenglish():
    ...
    savedkey.entryconfigure(0, label = "Key")
...
b3 = Button(master, textvariable = decryptbutton, command = lambda: decrypt2(e3.get("1.0","end"), 1), bg='white', borderwidth = 5).grid(row = 6, column = 3, ipadx = 1, stick = W)
b4 = Button(master, textvariable = copybutton, command = lambda: copy2(str(e5.get("1.0","end"))), bg='white', borderwidth = 5).grid(row = 9, column = 3, ipadx = 1, stick = W)
b6 = Button(master, textvariable = copybutton, command = lambda: copy4(str(e4.get("1.0","end"))), bg='white', borderwidth = 5).grid(row = 4, column = 3, ipadx = 1, stick = W)
...
Last edit made at 29/3/2022 01:38
"""
